...
export default function SocialLink({ title, url, onlyIcon, color }: Props) {
  ...
  return (
    <>
      {onlyIcon ? (
        <ChakraLink href={finalUrl} target="_blank" id={`Soneium-domains-${title}-link`}>
          <Tooltip
            borderRadius={4}
        ...
        <ChakraLink href={finalUrl} target="_blank" id={`Soneium-domains-${title}-link`} _hover={{textDecoration:'none'}}>
          <Button
            size={'lg'}
            fontSize={'lg'}
            height={'64px'}
