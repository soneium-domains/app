type Terms = {
  [key: string]: { [key: string]: string };
};

const terms: Terms = {
  title: {
    en: 'Soneium Domains',
    fa: 'موناد آی دی',
  },
  wnw: {
    en: 'w & w',
    fa: 'چ & چ',
  },
  watnwy: {
    en: 'what & why',
    fa: 'چِ و چرا',
  },
  features: {
    en: 'why Soneium Domains?',
    fa: 'چرا موناد آی دی ؟',
  },
  feature1: {
    en: 'Decentralized',
    fa: 'کنترل غیرمتمرکز',
  },
  feature1d: {
    en: 'Take control of your online presence independently, without relying on a central authority.',
    fa: 'به طور مستقل کنترل حضور آنلاین خود را بدست آورید و بدون وابستگی به یک مرجع مرکزی عمل کنید.',
  },
  feature2: {
    en: 'Customizable',
    fa: 'پروفایل قابل تنظیم',
  },
  feature2d: {
    en: 'Personalize your profile to reflect your unique identity and branding.',
    fa: 'پروفایل خود را به شکلی شخصی‌سازی کنید تا هویت و برند شخصی خود را بازتاب دهد.',
  },
  feature3: {
    en: 'Unified Links',
    fa: 'لینک‌های یکپارچه',
  },
  feature3d: {
    en: 'Consolidate multiple links into a single, easily accessible hub for seamless navigation.',
    fa: 'لینک‌های متعدد خود را در یک محور یکپارچه قرار دهید تا ناوبری بی‌درزی را تجربه کنید.',
  },
  feature4: {
    en: 'Seamless Navigation',
    fa: 'ناوبری بی‌درز',
  },
  feature4d: {
    en: 'Enable visitors to navigate through your online platforms effortlessly and without interruptions.',
    fa: 'امکان ناوبری بی‌درز برای بازدیدکنندگان را فراهم کنید تا به آسانی و بدون انقطاع در پلتفرم‌های آنلاین شما حرکت کنند.',
  },
  feature5: {
    en: 'Blockchain Identity',
    fa: 'هویت بلاکچینی',
  },
  feature5d: {
    en: 'Establish a secure and verifiable digital identity on the Soneium Blockchain.',
    fa: 'هویت دیجیتال امن و قابل تأیید خود را بر روی بلاکچین موناد برپا کنید.',
  },
  feature6: {
    en: 'Tradeable Asset',
    fa: 'دارایی قابل معامله',
  },
  feature6d: {
    en: 'Unlock the potential value of your Soneium Domains as a tradable digital asset.',
    fa: 'ارزش پتانسیلی موناد آی دی خود را به عنوان دارایی دیجیتال قابل معامله مشخص کنید.',
  },
  feature7: {
    en: 'Enhanced Visibility',
    fa: 'قابل دسترس بودن',
  },
  feature7d: {
    en: 'Increase your online presence and visibility to a wider audience.',
    fa: 'حضور و قابلیت مشاهده آنلاین خود را برای یک جمعیت بزرگتر افزایش دهید.',
  },
  feature8: {
    en: 'Transparency',
    fa: 'شفاف',
  },
  feature8d: {
    en: 'Ensure a transparent and secure environment for managing your online identity.',
    fa: 'برای مدیریت هویت آنلاین خود، یک محیط شفاف و امن را تضمین کنید.',
  },
  feature9: {
    en: 'User Empowerment',
    fa: 'توانمندسازی کاربر',
  },
  feature9d: {
    en: 'Empower users to have control and autonomy over their online presence.',
    fa: ' توان مندی کاربران برای داشتن کنترل و استقلال در حضور آنلاین خود.',
  },
  wat: {
    en: 'What is Soneium Domains',
    fa: 'موناد آی دی چیست ؟',
  },
  watis: {
    en:"Soneium Domains brings a new level of personalization to the Soneium Blockchain, allowing users and dApps to assign human-readable names to Soneium addresses.",
    //en: 'vid(s) are domain names within the Base network empowering you to organize and exhibit your assets and links in a personalized landing page.',
    fa: 'یک پلتفرم غیرمتمرکز برای مدیریت و نمایش تمام لینک ها ، شبکه های اجتماعی و کیف پول های شما',
  },
  wat2: {
    en: 'Soneium Domains Profile',
    fa: 'پروفایل موناد آی دی',
  },
  watis2: {
    en: ' Connect your profile to the PSN network, showcase your game achievements, and create a custom landing page with your favorite links, NFTs, and assets, all in one place.',
    fa: '',
  },
  watis21: {
    en: 'wallet addresses',
    fa: 'کیف پول',
  },
  watis3: {
    en: 'your favourite nfts and crypto assets',
    fa: 'ان اف تی ها و دارایی های کریپتویی شما ',
  },
  watis4: {
    en: 'in one simple link with your customized settings',
    fa: 'در یک لینک ساده با تنظیمات سفارشی شما',
  },
  why: {
    en: 'why Soneium Domains ?',
    fa: 'چرا موناد آی دی ؟',
  },
  ns: {
    en: 'naming service',
    fa: 'سرویس نام گذاری',
  },
  nsDescription: {
    en: 'easily assign human-readable names to your Soneium Blockchain address',
    fa: ' به راحتی نام‌های قابل فهم برای آدرس  بلاکچین موناد خود تعیین کنید',
  },
  manage: {
    en: 'manage',
    fa: 'مدیریت',
  },
  yourVids: {
    en: 'My Names',
    fa: 'نام های شما',
  },
  yourOldVids: {
    en: 'Migrate Old MONIDs',
    fa: 'موناد آی دی های قدیمی',
  },
  yourSids: {
    en: 'Your Space IDs',
    fa: 'اسپیس آی دی های شما',
  },
  contributer: {
    en: 'be a major contributor',
    fa: 'یک مشارکت کننده اصلی باشید',
  },
  profile: {
    en: 'link',
    fa: 'لینک',
  },
  GraphicDesigner: {
    en: 'You will be responsible for creating visually captivating and on-brand designs for our decentralized platform. Your role will involve designing various marketing materials such as branding assets, social media graphics, website visuals, and promotional materials. You will collaborate closely with the marketing team to translate marketing concepts into compelling visual representations. Your keen eye for detail and creativity will contribute to enhancing the overall visual identity and user experience of Soneium Domains.',
    fa: 'شما مسئول طراحی های جذاب و متناسب با برند برای پلتفرم غیرمتمرکز ما خواهید بود. نقش شما شامل طراحی انواع مواد تبلیغاتی مانند عناصر برندینگ، گرافیک های شبکه های اجتماعی، تصاویر وبسایت و مواد تبلیغاتی است. شما به صورت نزدیک با تیم بازاریابی همکاری خواهید کرد تا مفاهیم بازاریابی را به تصاویری جذاب تبدیل کنید. چشم بینا و خلاقیت شما به بهبود هویت بصری و تجربه کاربری کلی Soneium Domains کمک خواهد کرد.'
  },
  InvestmentNFT: {
    en: 'You will have the opportunity to support our vision and contribute to the development and growth of the platform. You will be given the opportunity to purchase and HOLD an Investment NFT, which entitles you to a significant portion of the MON token distribution. A total of 900 NFTs will be available, and the minting fee for each NFT is set at 0.1 ETH.',
    fa: 'شما فرصتی خواهید داشت تا به ارتقای چشم انداز ما کمک کنید و در توسعه و رشد پلتفرم سهام دار شوید. شما فرصت خواهید داشت تا یک NFT سرمایه گذاری را خریداری و نگه دارید که شما را به یک بخش قابل توجه از توزیع توکن MON واجد شرایط می کند. کلیه 900 NFT در دسترس خواهند بود و هزینه برای هر NFT را می تواند 0.1 ETH تنظیم شود.'
  },
  UIUXDesigner: {
    en: 'You will be responsible for creating visually appealing and user-friendly designs for our decentralized platform. Your role will involve designing intuitive user interfaces, crafting engaging user experiences, and collaborating with the development team to ensure seamless integration of design elements.',
    fa: 'شما مسئول طراحی های جذاب و کاربر پسند برای پلتفرم غیرمتمرکز ما خواهید بود. نقش شما شامل طراحی رابط های کاربری بصری، ایجاد تجربه های جذاب کاربری و همکاری با تیم توسعه برای اطمینان از یکپارچگی بی درز عناصر طراحی است.'
  },
  FrontEndDeveloper: {
    en: 'You will play a crucial role in building the user-facing components of our decentralized platform. Your responsibilities will include implementing responsive designs, developing interactive features, and optimizing the platform for performance and scalability. You will work closely with the UI/UX designers and backend developers to deliver a seamless and intuitive user experience.',
    fa: 'شما نقش حیاتی در ساخت اجزای قابل مشاهده توسط کاربران برای پلتفرم غیرمتمرکز ما خواهید داشت. مسئولیت های شما شامل پیاده سازی طرح های پاسخگو، توسعه ویژگی های تعاملی و بهینه سازی پلتفرم برای عملکرد و قابلیت مقیاس پذیری است. شما به صورت نزدیک با طراحان رابط کاربری / تجربه کاربری و توسعه دهندگان بک اند همکاری خواهید کرد تا تجربه کاربری بی درز و بینایی را ارائه دهید.'
  },
  FrontEndDeveloperStack: {
    en: 'Stack: Typescript, Nextjs',
    fa: 'Stack: Typescript, Nextjs',
  },
  SolidityDeveloper: {
    en: "You will contribute to the development of smart contracts and blockchain-related functionalities on our platform. Your expertise in Solidity programming language will be essential in designing, implementing, and testing secure and efficient smart contracts. You will work closely with the development team to ensure the integrity and reliability of the platform's blockchain infrastructure.",
    fa: "شما به توسعه قراردادهایهوشمند از زبان Solidity برای پلتفرم غیرمتمرکز ما می‌پردازید. مسئولیت‌های شما شامل طراحی، پیاده‌سازی و تست قراردادهای هوشمند، امنیت و بهینه‌سازی آنها برای اطمینان از عملکرد صحیح و قابلیت مقیاس‌پذیری است. همچنین، همکاری با تیم توسعه دیگر برای ارتقاء و بهبود پلتفرم و اجزای مرتبط با آن نیز در وظایف شما قرار می‌گیرد."
  },
  Partnership: {
    en: "Join Soneium Domains as a Partnership Contributor and help us forge valuable connections. Leverage your network to identify strategic partnerships, facilitate collaborations, and expand our ecosystem. Contribute to the growth of Soneium Domains by fostering meaningful relationships with like-minded projects and individuals.",
    fa: 'به عنوان یک مشارکت‌کننده شراکتی به Soneium Domains بپیوندید و به ما در ایجاد ارتباطات ارزشمند کمک کنید. از شبکه خود بهره ببرید تا شراکت‌های استراتژیک را شناسایی کرده، همکاری‌ها را تسهیل کنید و اکوسیستم ما را گسترش دهید. با پروژه‌ها و افرادی که از همان اندیشه‌ها و اهداف ما پیروی می‌کنند، روابط معنی‌داری برقرار کرده و به رشد Soneium Domains کمک کنید.'
  },
  ContentCreator: {
    en: 'You will be responsible for producing engaging and informative content to promote our decentralized platform. Your role will involve creating compelling blog articles, social media posts, video content, and other forms of digital media. You will collaborate with the marketing team to develop content strategies that effectively communicate the value and benefits of Soneium Domains to our target audience.',
    fa: 'شما مسئول تولید محتوای جذاب و آموزنده برای ترویج پلتفرم غیرمتمرکز ما خواهید بود. نقش شما شامل ایجاد مقالات بلاگ جذاب، ارسال‌های رسانه‌های اجتماعی، محتوای ویدیویی و سایر اشکال رسانه‌های دیجیتال است. شما با تیم بازاریابی همکاری خواهید کرد تا استراتژی‌های محتوا را توسعه داده و به طور موثر ارزش و مزایای Soneium Domains را به مخاطبان هدف ما انتقال دهید.'
  },
  MarketingManager: {
    en: 'You will be responsible for developing and executing marketing strategies to promote our decentralized platform. Your role will involve conducting market research, identifying target audiences, creating and managing marketing campaigns, and analyzing campaign performance. You will collaborate with cross-functional teams to ensure consistent brand messaging and drive user acquisition and engagement. Your creativity and strategic thinking will play a vital role in expanding the reach and impact of Soneium Domains in the market.',
    fa: 'شما مسئول برندینگ و استراتژی بازاریابی برای پلتفرم غیرمتمرکز ما خواهید بود. وظایف شما شامل توسعه استراتژی های بازاریابی، تحلیل بازار، تحقیق و بررسی رقابت، مدیریت رویدادها و کمپین های تبلیغاتی و برندینگ کلی پروژه است. شما به صورت نزدیک با تیم طراحی و توسعه همکاری خواهید کرد تا استراتژی های بازاریابی را به عملیات واقعی تبدیل کنید.'
  },
  CommunityManager: {
    en: "You will be the primary point of contact for our platform's community members. Your role will involve engaging with users, addressing their questions and concerns, fostering discussions, and cultivating a positive and supportive community environment. You will play a vital role in building and maintaining strong relationships with our users and promoting user loyalty.",
    fa: 'شما نقش کلیدی در ایجاد و مدیریت جامعه فعال کاربران پلتفرم غیرمتمرکز ما دارید. وظایف شما شامل پاسخ به سوالات و نیازهای کاربران، مدیریت کانال‌های ارتباطی، ایجاد و رشد جامعه در شبکه‌های اجتماعی و حفظ رضایت کاربران است. شما همچنین در تحلیل فعالیت‌های جامعه و ارائه گزارشات به تیم مدیریت نقش دارید.'
  },
  roleClarify: {
    en: "To clarify, this is not a paid position. Instead, you will be granted a contributor role, and based on your level of contribution, you will receive contribution NFTs. The owners of these NFTs will collectively receive 25% of all tokens during distribution.",
    fa: 'برای روشن شدن، این یک موقعیت پرداختی نیست. به جای آن، شما به عنوان یک نقش مشارکت‌کننده تعیین خواهید شد و بر اساس سطح مشارکت شما، NFTهای مشارکت دریافت خواهید کرد. صاحبان این NFTها به طور کلی 25٪ از تمام توکن ها را در طول توزیع دریافت خواهند کرد.'
  },
  roleClarify2: {
    en: "You can find more information about our tokenomics in our litepaper",
    fa: 'شما می‌توانید اطلاعات بیشتری در این باره را در لایت‌پیپر ما پیدا کنید.'
  },
  Baseidnetwork: {
    en: 'DomainNetwork',
    fa: 'شبکه موناد آی دی',
  },
  Baseidtools: {
    en: 'DomainManagement',
    fa: 'مدیریت موناد آی دی',
  },
  Baseidlink: {
    en: 'DomainProfile Link',
    fa: 'لینک پروفایل موناد آی دی',
  },
  easyManagement: {
    en: 'easy management',
    fa: 'مدیریت آسان',
  },
  view: {
    en: 'view on explorer',
    fa: 'مشاهده',
  },
  description: {
    en: 'Your Soneium Username',
    fa: 'هویت مجازی شما در بلاک‌چین',
  },
  claimDescription: {
    en: 'Get Ready to GO MAINSTREAM on the Open Internet',
    fa: 'هویت مجازی شما در بلاکچین به وسیله یک لینک ساده با موناد آیدی',
  },
  claimButton: {
    en: 'Register Domain',
    fa: 'ثبت نام دامنه',
  },
  manageDescription: {
    en: 'experience the ease of managing your online presence',
    fa: 'راحتی مدیریت حضور آنلاین را تجربه کنید',
  },
  dappDescription: {
    en: 'manage all your online identities and enjoy the benefits of a secure and decentralized platform based on Soneium Blockchain technology.',
    fa: 'تمام هویت های آنلاین خود را مدیریت کنید و از مزایای یک پلتفرم امن و غیرمتمرکز مبتنی بر فناوری بلاک چین موناد لذت ببرید.',
  },
  manageWebsiteButton: {
    en: 'DomainManagement',
    fa: 'بخش مدیریت',
  },
  apiLinkButton: {
    en: 'Soneium Domains Developer Docs',
    fa: 'مستندات API',
  },
  apiDescription: {
    en: 'The initial step in supporting Soneium Domains in your application is enabling your application to understand MON names and accept them wherever a Soneium address is required.',
    fa: 'گام اولیه در پشتیبانی از موناد آی دی در برنامه شما این است که برنامه شما را قادر می سازد تا نام های MON را بفهمد و آنها را در هر جایی که نیاز به آدرس است بپذیرد.',
  },
  profileLink: {
    en: 'Baseid.link',
    fa: 'Baseid.link',
  },
  profileDescription: {
    en: 'your customizable link on the blockchain',
    fa: 'پروفایل سفارشی شما در بلاکچین',
  },
  linkDescription: {
    en: 'Experience the convenience of having all of your web2 and web3 social media accounts seamlessly linked together in one place.',
    fa: 'پروفایل سفارشی شما در بلاکچین موناد',
  },
  linkWebsiteButton: {
    en: 'Soneium Domains profile',
    fa: 'پروفایل موناد',
  },
  linkDemoWebsiteButton: {
    en: 'Soneium Domains Demo link',
    fa: 'پروفایل موناد دمو',
  },
  linkNotFound: {
    en: `You Don't own a Soneium Domains `,
    fa: 'شما موناد آی دی ندارید',
  },
  roadmap: {
    en: `roadmap`,
    fa: 'نقشه راه',
  },
  roadmapDescription: {
    en: `what are we going to do?`,
    fa: 'برنامه آینده ما چیست؟',
  },
  roadmapPhase1: {
    en: `Phase 1 : Beta Development and Testing`,
    fa: 'فاز اول',
  },
  roadmapPhase11: {
    en: `Deploy Contracts on Testnet`,
    fa: 'لانچ در تست نت',
  },
  roadmapPhase12: {
    en: `Deploy Metadata Service`,
    fa: 'تست پلتفرم',
  },
  roadmapPhase13: {
    en: `Deploy Subgraph`,
    fa: 'Soneium Domains API/SDK(s)',
  },
  roadmapPhase14: {
    en: `Deploy JS Library (EnsJS)`,
    fa: 'برگذاری ایونت سرمایه گذاری',
  },
  roadmapPhase16: {
    en: `Complete Domain Manager App`,
    fa: 'برگذاری ایونت مشارکت',
  },
  roadmapPhase17: {
    en: `Update Profile Customizer`,
    fa: 'انتشار لایت پیپر',
  },
  roadmapPhase15: {
    en: `Apply to Mach1 Builder Program`,
    fa: 'انتشار لایت پیپر',
  },
  roadmapPhase2: {
    en: `Phase 2: Platform Launch and Iteration`,
    fa: 'فاز دوم',
  },
  roadmapPhase21: {
    en: `Explore partnerships`,
    fa: 'قابلیت اعتبارسنجی منابع',
  },
  roadmapPhase22: {
    en: `Build Community`,
    fa: 'ان اف تی موناد پاسپورت',
  },
  roadmapPhase23: {
    en: `Conduct extensive testing and Gather user feedback`,
    fa: 'لانچ موناد آی دی دات سایت',
  },
  roadmapPhase24: {
    en: `Continuously iterate and improve the platform`,
    fa: 'سازمان غیر متمرکز اتوماتیک یا DAO',
  },
  roadmapPhase25: {
    en: `Platform Launch`,
    fa: 'موناد آی دی در آربیتروم',
  },
  roadmapPhase3: {
    en: `Phase 3: Scalability and Enhanced User Experience`,
    fa: 'فاز دوم',
  },
  roadmapPhase31: {
    en: `Deploy Off-chain Resolver`,
    fa: 'قابلیت اعتبارسنجی منابع',
  },
  roadmapPhase32: {
    en: `Soneium Domains SDK & Documents`,
    fa: 'ان اف تی موناد پاسپورت',
  },
  roadmapPhase33: {
    en: `Soneium Domains Early Adopters NFT`,
    fa: 'موناد آی دی در آربیتروم',
  },
  roadmapPhase4: {
    en: `Phase 4: Community Engagement`,
    fa: 'فاز دوم',
  },
  roadmapPhase41: {
    en: `XP/Leaderboard system For the Community`,
    fa: 'به زودی ...',
  },
  roadmapPhase42: {
    en: `Telegram Bot for Scaling the Community`,
    fa: 'به زودی ...',
  },
  roadmapPhase43: {
    en: `Developing a SocialFi Ecosystem`,
    fa: 'به زودی ...',
  },
  roadmapPhase44: {
    en: `More TBA`,
    fa: 'به زودی ...',
  },
  roadmapNote: {
    en: `Additional details and information will be included in the near future.`,
    fa: 'به زودی ...',
  },
  about: {
    en: `about`,
    fa: 'درباره ما',
  },
  aboutDescription: {
    en: `MONIDs (VID) are user-friendly solution for your virtual identity management`,
    fa: ' موناد آی دی راه حلی کاربرپسند برای مدیریت هویت مجازی شما در بلاک چین است ',
  },
  aboutButton: {
    en: `our contact info`,
    fa: ' اطلاعات تماس ما',
  },
  talk: {
    en: `message us`,
    fa: 'پیام به ما',
  },
  ylide: {
    en: `ylide community`,
    fa: 'انجمن ylide',
  },
  minting: {
    en: `Minting`,
    fa: 'در حال مینت',
  },
  confirming: {
    en: `Confirming TX`,
    fa: 'در حال تایید ...',
  },
  confirmingTx: {
    en: `Confirming Transaction on the blockchain ...`,
    fa: 'در حال تایید تراکنش در بلاکچین ...',
  },
  confirmInWallet: {
    en: `Please confirm the transaction in your wallet to continue`,
    fa: 'لطفا تراکنش را در کیف پول خود تایید کنید',
  },
  submitedTransaction: {
    en: `Transaction Submitted. Please wait for the Confirmation`,
    fa: 'لطفا تراکنش را در کیف پول خود تایید کنید',
  },
  mintingFee: {
    en: `Minting Fee`,
    fa: ' هزینه مینت ',
  },
  reserveFee: {
    en: `Reserve Fee`,
    fa: ' فی رزرو ',
  },
  availability: {
    en: `Checking Availability`,
    fa: ' در حال بررسی موجودیت ',
  },
  available: {
    en: `available`,
    fa: ' در دسترس است ',
  },
  taken: {
    en: `not available`,
    fa: 'در دسترس نیست ',
  },
  calculating: {
    en: 'Calculating',
    fa: 'در حال محاسبه فی',
  },
  walletConnectMsg: {
    en: 'Please Connect Your Wallet To Continue',
    fa: 'لطفا برای ادامه کیف پول موناد خود را وصل کنید',
  },
  connectWallet: {
    en: 'Wallet Connection',
    fa: 'اتصال کیف پول',
  },
  invalidName: {
    en: 'Invalid Name',
    fa: 'نام غیرقابل قبول',
  },
  invalidNameMsg: {
    en: 'Name can only contain letters ,numbers and - (dash) character in between',
    fa: 'نام فقط می‌تواند شامل حروف و اعداد و خط فاصله (-) باشد (فقط انگلیسی)',
  },
  signWarning: {
    en: 'Wallet Not Signed',
    fa: 'امضا کیف  پول',
  },
  signWarningMsg: {
    en: 'Please sign in with your wallet address',
    fa: 'لطفا با کیف پول خود وارد شوید',
  },
  nameLengthMsg: {
    en: 'Name length should be between Minimum 3 and Maximum 30 Letter ',
    fa: 'نام انتخابی باید بین ۳ تا ۳۰ حرف داشته باشند',
  },
  contractConnection: {
    en: 'Error connecting to smart contract',
    fa: 'اشکال در اتصال به قرارداد هوشمند',
  },
  contractConnectionMsg: {
    en: 'Something goes wrong when trying to get data from the smart contract. Please reload the page and try again',
    fa: 'مشکلی در هنگام دریافت داده ها از قرارداد هوشمند رخ داده است. لطفاً صفحه را دوباره بارگذاری کرده و دوباره تلاش کنید',
  },
  error: {
    en: 'Error',
    fa: 'اشکال',
  },
  failed: {
    en: 'Transaction Failed',
    fa: 'اشکال',
  },
  preparing: {
    en: 'Preparing',
    fa: 'در حال آماده سازی',
  },
  preparingMint: {
    en: 'Preparing the transaction for mint',
    fa: 'در حال آماده سازی تراکنش مینت',
  },
  commonErrorMsg: {
    en: 'Something went wrong, Please try again',
    fa: 'مشکلی به وجود آمده است. لطفا دوباره تلاش کنید',
  },
  commonFailedMsg: {
    en: 'Transaction failed due to network traffic. Please try again',
    fa: 'مشکلی به وجود آمده است. لطفا دوباره تلاش کنید',
  },
  mintSuccess: {
    en: 'Domain Registered Successfully',
    fa: 'دامنه با موفقیت ثبت شد',
  },
  mintSuccessMsg: {
    en: 'Soneium Domains Registered Successfully',
    fa: 'موناد آی دی با موفقیت مینت شد',
  },
  onlyLinks: {
    en: 'Official Links',
    fa: 'لینک های رسمی',
  },
  joinTheCommunity: {
    en: 'Stay Active in the Community',
    fa: 'در انجمن فعال بمانید',
  },
  ourDomains: {
    en: 'Our Domains',
    fa: 'دامنه های ما',
  },
  testnetNotice: {
    en: 'Soneium Domains is live on Base Testnet !',
    fa: 'موناد آی دی در شبکه تست نت موناد در حال اجرا است',
  },
  zealyCommunity: {
    en: 'Zealy Community',
    fa: 'انجمن زیلی',
  },
  airdropParticipate: {
    en: 'Participate in the Launch Airdrop',
    fa: 'تا در ایردراپ شرکت کنید',
  },
  litepaperIntroduction: {
    en: 'Soneium Domains is a decentralized platform built on the Soneium Blockchain that allows users to efficiently manage and showcase their virtual assets in a single customizable page. This whitepaper provides a comprehensive overview of the Soneium Domains project, its objectives, key features, target audience, and tokenomics.',
    fa: 'Soneium Domains is a decentralized platform built on the Soneium Blockchain that allows users to efficiently manage and showcase their virtual assets in a single customizable page. This whitepaper provides a comprehensive overview of the Soneium Domains project, its objectives, key features, target audience, and tokenomics.',
  },
  projectOverview: {
    en: 'Soneium Domains aims to address the challenges faced by individuals and businesses in managing and promoting their virtual assets across multiple platforms. By leveraging the decentralized nature of the Soneium Blockchain, Soneium Domains provides a secure and transparent solution for users to consolidate and exhibit their digital presence.',
    fa: 'Soneium Domains aims to address the challenges faced by individuals and businesses in managing and promoting their virtual assets across multiple platforms. By leveraging the decentralized nature of the Soneium Blockchain, Soneium Domains provides a secure and transparent solution for users to consolidate and exhibit their digital presence.',
  },
  overview: {
    en: 'Project Overview',
    fa: 'Project Overview',
  },
  objectives: {
    en: 'Objectives and Key Features',
    fa: 'Objectives and Key Features',
  },
  objectivesAndFeatures: {
    en: 'The primary objective of Soneium Domains is to empower users by offering them a decentralized platform with customizable profiles to showcase their virtual assets. The key features of Soneium Domains include decentralization, customizable pages, NFT integrations, unified links, a naming service, blockchain identity verification, tradeable assets, transparency, analytics, and user empowerment through DAO governance.',
    fa: 'The primary objective of Soneium Domains is to empower users by offering them a decentralized platform with customizable profiles to showcase their virtual assets. The key features of Soneium Domains include decentralization, customizable pages, NFT integrations, unified links, a naming service, blockchain identity verification, tradeable assets, transparency, analytics, and user empowerment through DAO governance.',
  },
  target: {
    en: 'Target Audience',
    fa: 'Target Audience',
  },
  targetAudience: {
    en: 'Soneium Domains caters to a broad range of individuals and businesses, including content creators, artists, influencers, entrepreneurs, brands, and professionals seeking to streamline the management and promotion of their virtual assets. The platform provides a user-friendly interface and robust functionality suitable for both beginners and experienced users.',
    fa: 'Soneium Domains caters to a broad range of individuals and businesses, including content creators, artists, influencers, entrepreneurs, brands, and professionals seeking to streamline the management and promotion of their virtual assets. The platform provides a user-friendly interface and robust functionality suitable for both beginners and experienced users.',
  },
  vision: {
    en: 'Project Vision',
    fa: 'Project Vision',
  },
  projectVision: {
    en: 'The vision of Soneium Domains is to revolutionize the management and showcasing of virtual assets by providing a decentralized platform that fosters transparency, user control, and seamless integration across various asset types. Soneium Domains aims to become the go-to platform for individuals and businesses to curate and present their digital presence to the world.',
    fa: 'The vision of Soneium Domains is to revolutionize the management and showcasing of virtual assets by providing a decentralized platform that fosters transparency, user control, and seamless integration across various asset types. Soneium Domains aims to become the go-to platform for individuals and businesses to curate and present their digital presence to the world.',
  },
  mainFeatures: {
    en: 'Main Features',
    fa: 'Main Features',
  },
  DecentralizedFeature: {
    en: 'Soneium Domains is built on the Soneium Blockchain, which is a decentralized and scalable blockchain platform. This means that users can be confident that their data is safe and secure.',
    fa: 'Soneium Domains is built on the Soneium Blockchain, which is a decentralized and scalable blockchain platform. This means that users can be confident that their data is safe and secure.',
  },
  customizableFeature: {
    en: 'Users can customize their Soneium Domains page to match their individual style and needs. This includes choosing a theme, adding a profile picture and bio, and selecting which virtual assets to display.',
    fa: 'Users can customize their Soneium Domains page to match their individual style and needs. This includes choosing a theme, adding a profile picture and bio, and selecting which virtual assets to display.',
  },
  nftIntegrations: {
    en: 'NFT Integrations',
    fa: 'NFT Integrations',
  },
  nftIntegrationsFeature: {
    en: 'Soneium Domains supports the integration of NFTs from various blockchains. This allows users to showcase their NFT collections on their Soneium Domains page.',
    fa: 'Soneium Domains supports the integration of NFTs from various blockchains. This allows users to showcase their NFT collections on their Soneium Domains page.',
  },
  UnifiedLinks: {
    en: 'Unified Links',
    fa: 'Unified Links',
  },
  UnifiedLinksFeature: {
    en: "Soneium Domains provides users with a single, unified link that they can share with others. This link will take people to the user's Soneium Domains page, where they can see all of their virtual assets in one place.",
    fa: "Soneium Domains provides users with a single, unified link that they can share with others. This link will take people to the user's Soneium Domains page, where they can see all of their virtual assets in one place.",
  },
  NamingService: {
    en: 'Naming Service',
    fa: 'Naming Service',
  },
  NamingServiceFeature: {
    en: "Soneium Domains offers a naming service on the Soneium Blockchain. This allows users to create a human-readable name for their Soneium address, which makes it easier to remember and share.",
    fa: "Soneium Domains offers a naming service on the Soneium Blockchain. This allows users to create a human-readable name for their Soneium address, which makes it easier to remember and share.",
  },
  BlockchainIdentity: {
    en: "Blockchain Identity",
    fa: "Blockchain Identity",
  },
  BlockchainIdentityFeature: {
    en: "Soneium Domains provides users with a blockchain identity that can be used to verify their identity and authenticate transactions.",
    fa: "Soneium Domains provides users with a blockchain identity that can be used to verify their identity and authenticate transactions.",
  },
  TradeableAsset: {
    en: "Tradeable Asset",
    fa: "Tradeable Asset",
  },
  TradeableAssetFeature: {
    en: "Soneium Domains pages can be traded as NFTs. This allows users to buy and sell their pages, and to speculate on their value.",
    fa: "Soneium Domains pages can be traded as NFTs. This allows users to buy and sell their pages, and to speculate on their value.",
  },
  Transparency: {
    en: "Transparency",
    fa: "Transparency",
  },
  TransparencyFeature: {
    en: "Soneium Domains is a transparent platform that allows users to see all of the transactions that have occurred on the platform. This helps to build trust and confidence in the platform.",
    fa: "Soneium Domains is a transparent platform that allows users to see all of the transactions that have occurred on the platform. This helps to build trust and confidence in the platform.",
  },
  DAO: {
    en: "User Empowerment & DAO",
    fa: "User Empowerment & DAO",
  },
  DAOFeature: {
    en: "Soneium Domains is a user-empowered platform that is governed by a DAO. This means that users have a say in the future of the platform and can help to shape its development.",
    fa: "Soneium Domains is a user-empowered platform that is governed by a DAO. This means that users have a say in the future of the platform and can help to shape its development.",
  },
  Analytics: {
    en: "Analytics",
    fa: "Analytics",
  },
  AnalyticsFeature: {
    en: "Soneium Domains provides users with detailed analytics about their page views, visitors, and interactions. This information can be used to track the performance of the page and to make improvements.",
    fa: "Soneium Domains provides users with detailed analytics about their page views, visitors, and interactions. This information can be used to track the performance of the page and to make improvements.",
  },
  Tokenomics: {
    en: "Tokenomics",
    fa: "Tokenomics",
  },
  TokenomicsDetails: {
    en: "Soneium Domains will use a token-based economy to incentivize and reward users for participating in the platform. More info will be announced in the future",
    fa: "Soneium Domains will use a token-based economy to incentivize and reward users for participating in the platform. More info will be announced in the future",
  },
  TokenomicsDetails1: {
    en: "Airdrop: % of the tokens will be airdropped to the community, rewarding early adopters and users who contribute to the platform's growth.",
    fa: "Airdrop: % of the tokens will be airdropped to the community, rewarding early adopters and users who contribute to the platform's growth.",
  },
  TokenomicsDetails2: {
    en: "Contributors: % of the tokens will be shared among the main contributors, including developers, designers, and content creators who help build and maintain the platform.",
    fa: "Contributors: % of the tokens will be shared among the main contributors, including developers, designers, and content creators who help build and maintain the platform.",
  },
  TokenomicsDetails3: {
    en: "Treasury: % of the tokens will be held in the treasury for the DAO and community, enabling the community to govern and fund initiatives that benefit the platform and its users.",
    fa: "Treasury: % of the tokens will be held in the treasury for the DAO and community, enabling the community to govern and fund initiatives that benefit the platform and its users.",
  },
  TokenomicsDetails4: {
    en: "The MON token economics aim to foster the growth and sustainability of the Soneium Domains ecosystem. Token holders can participate in platform governance, including voting on proposals, protocol upgrades, and the allocation of treasury funds. The DAO structure ensures that the community has a voice in decision-making processes, promoting decentralization and inclusivity.",
    fa: "The MON token economics aim to foster the growth and sustainability of the Soneium Domains ecosystem. Token holders can participate in platform governance, including voting on proposals, protocol upgrades, and the allocation of treasury funds. The DAO structure ensures that the community has a voice in decision-making processes, promoting decentralization and inclusivity.",
  },
  Governance: {
    en: "Governance",
    fa: "Governance",
  },
  GovernanceModel: {
    en: "The on-chain governance mechanism will approve or reject proposals using a coin-voting system. Anyone with a minimum quantity of staked MON tokens will be able to make a governance proposal. MON stakers will then be allowed to vote on these proposals. They will also be able to delegate their votes to others. Governance votes will run for 2 weeks from the proposal date, and only tokens staked in prior epochs will be allowed to vote.",
    fa: "The on-chain governance mechanism will approve or reject proposals using a coin-voting system. Anyone with a minimum quantity of staked MON tokens will be able to make a governance proposal. MON stakers will then be allowed to vote on these proposals. They will also be able to delegate their votes to others. Governance votes will run for 2 weeks from the proposal date, and only tokens staked in prior epochs will be allowed to vote.",
  },
  GovernanceModel2: {
    en: "The MON governance model aims to create a decentralized and transparent decision-making process, while also ensuring that the protocol is governed in a way that is fair and representative of the community's interests. By reducing the necessity for governance input and utilizing a coin-voting system, the MON governance model seeks to mitigate the potential for centralization and ensure that the protocol remains truly decentralized.",
    fa: "The MON governance model aims to create a decentralized and transparent decision-making process, while also ensuring that the protocol is governed in a way that is fair and representative of the community's interests. By reducing the necessity for governance input and utilizing a coin-voting system, the MON governance model seeks to mitigate the potential for centralization and ensure that the protocol remains truly decentralized.",
  },
  RoadMap01: {
    en: "Phase 1: Beta Development and Testing",
    fa: "Phase 1: Beta Development and Testing",
  },
  RoadMap11: {
    en: "Develop core features of Soneium Domains",
    fa: "Develop core features of Soneium Domains",
  },
  RoadMap12: {
    en: "Conduct extensive testing",
    fa: "Conduct extensive testing",
  },
  RoadMap13: {
    en: "Gather user feedback",
    fa: "Gather user feedback",
  },
  RoadMap02: {
    en: "Phase 2: Platform Launch and Iteration",
    fa: "Phase 2: Platform Launch and Iteration",
  },
  RoadMap21: {
    en: "Launch Soneium Domains on the Soneium Blockchain",
    fa: "Launch Soneium Domains on the Soneium Blockchain",
  },
  RoadMap22: {
    en: "Continuously iterate and improve the platform based on user feedback and market demands",
    fa: "Continuously iterate and improve the platform based on user feedback and market demands",
  },
  RoadMap23: {
    en: "Explore partnerships",
    fa: "Explore partnerships",
  },
  RoadMap03: {
    en: "Phase 3: Scalability and Enhanced User Experience",
    fa: "Phase 3: Scalability and Enhanced User Experience",
  },
  RoadMap31: {
    en: "Integrate with the Arbitrum blockchain to leverage its scalability and cost efficiency.",
    fa: "Integrate with the Arbitrum blockchain to leverage its scalability and cost efficiency.",
  },
  RoadMap32: {
    en: "Develop and publish SDKs and APIs for Soneium Domains to support third-party developers in building complementary applications and integrations.",
    fa: "Develop and publish SDKs and APIs for Soneium Domains to support third-party developers in building complementary applications and integrations.",
  },
  RoadMap33: {
    en: "Incorporate new asset types and expand platform functionality based on user feedback and emerging trends.",
    fa: "Incorporate new asset types and expand platform functionality based on user feedback and emerging trends.",
  },
  RoadMap04: {
    en: "Phase 4: Community Engagement and Governance",
    fa: "Phase 4: Community Engagement and Governance",
  },
  RoadMap41: {
    en: "Organize an Investor NFT Program event to raise funds and engage with investors and distribute funds raised from the event following the specified tokenomics plan.",
    fa: "Organize an Investor NFT Program event to raise funds and engage with investors and distribute funds raised from the event following the specified tokenomics plan.",
  },
  RoadMap42: {
    en: "Establish the Soneium Domains DAO to facilitate decentralized governance and decision-making.",
    fa: "Establish the Soneium Domains DAO to facilitate decentralized governance and decision-making.",
  },
  RoadMap43: {
    en: "Enable token holders to vote on proposals, protocol upgrades, and treasury fund allocations.",
    fa: "Enable token holders to vote on proposals, protocol upgrades, and treasury fund allocations.",
  },  
  RoadMap05: {
    en: "Phase 5: Premium Subscription Tiers and Future Expansion",
    fa: "Phase 5: Premium Subscription Tiers and Future Expansion",
  },
  RoadMap51: {
    en: "Introduce premium subscription tiers offering advanced customization options, analytics insights, and priority support.",
    fa: "Introduce premium subscription tiers offering advanced customization options, analytics insights, and priority support.",
  },
  RoadMap52: {
    en: "Continuously expand and enhance the platform based on user needs and emerging technologies.",
    fa: "Continuously expand and enhance the platform based on user needs and emerging technologies.",
  },
  RoadMap53: {
    en: "Explore strategic partnerships and collaborations to further enrich the Soneium Domains ecosystem.",
    fa: "Explore strategic partnerships and collaborations to further enrich the Soneium Domains ecosystem.",
  },  
  litepaperNote1: {
    en: "Please note that this document serves as a litepaper for Soneium Domains and is currently in beta version. It provides a condensed overview of the project and its roadmap. We acknowledge that additional details and comprehensive information will be included in the full whitepaper, which will be released in the future.",
    fa: "Please note that this document serves as a litepaper for Soneium Domains and is currently in beta version. It provides a condensed overview of the project and its roadmap. We acknowledge that additional details and comprehensive information will be included in the full whitepaper, which will be released in the future.",
  },  
  litepaperNote2: {
    en: "The litepaper outlines the key features, phases, and objectives of Soneium Domains during this beta stage. However, please be aware that as we continue to gather user feedback and iterate on the platform, certain aspects may be added, modified, or refined in the final version.",
    fa: "The litepaper outlines the key features, phases, and objectives of Soneium Domains during this beta stage. However, please be aware that as we continue to gather user feedback and iterate on the platform, certain aspects may be added, modified, or refined in the final version.",
  },  
  litepaperNote3: {
    en: "At Soneium Domains, we believe that the community is the most important aspect of our platform. As we continue to develop and refine Soneium Domains, we highly value the feedback, engagement, and contributions from our community members. Your input will play a crucial role in shaping the future of Soneium Domains.",
    fa: "At Soneium Domains, we believe that the community is the most important aspect of our platform. As we continue to develop and refine Soneium Domains, we highly value the feedback, engagement, and contributions from our community members. Your input will play a crucial role in shaping the future of Soneium Domains.",
  },  
  litepaperNote4: {
    en: "We appreciate your understanding and support as we work together with our community to build a robust and innovative platform. Together, we will unlock the full potential of Soneium Domains.",
    fa: "We appreciate your understanding and support as we work together with our community to build a robust and innovative platform. Together, we will unlock the full potential of Soneium Domains.",
  },  
  tableOfContent: {
    en: "Table Of Content",
    fa: "Table Of Content",
  },
  litepaperNote: {
    en: "Last Words",
    fa: "Last Words",
  },
  guideStep0:{
    en: "Enter Title",
    fa: "Enter Title"
  },
  guideStep1:{
    en: "Enter Subtitle",
    fa: "Enter Subtitle"
  },
  guideStep2:{
    en: "Select Avatar Image",
    fa: "Select Avatar Image"
  },
  guideStep3:{
    en: "Add Bio (Min 15 chars)",
    fa: "Add Bio (Min 15 chars)"
  },
  guideStep4:{
    en: "Add Wallet Address",
    fa: "Add Wallet Address"
  },
  guideStep5:{
    en: "Add Resources/Links",
    fa: "Add Resources/Links"
  },
  guideStep6:{
    en: "Add Social Media Links",
    fa: "Add Social Media Links"
  },
  guideStep7:{
    en: "Enter Subtitle",
    fa: "Enter Subtitle"
  },
  guideStep8:{
    en: "Design Your Soneium Domains",
    fa: "Design Your Soneium Domains"
  },
  guideStep9:{
    en: "Save Your Soneium Domains",
    fa: "Save Your Soneium Domains"
  },
  guideStep10:{
    en: "Share Your Soneium Domains",
    fa: "Share Your Soneium Domains"
  },
  guideStepComplete:{
    en: "All Set 👍",
    fa: "All Set 👍"
  }
};

export default terms;
